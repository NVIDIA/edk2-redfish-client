...
GenChangePasswordCs (
  ...
  json_t                                *JsonObj,
  char                                  *Key,
  RedfishBios_V1_2_0_ChangePassword_CS  **Dst
  )
{
  ...
  if ((Status != RedfishCS_status_success) && (Status != RedfishCS_status_not_found)) {
    goto Error;
  } else {
    if (Status == RedfishCS_status_not_found) {
      /*This is not the required property.*/
    }
  }
...
GenResetBiosCs (
  ...
  json_t                           *JsonObj,
  char                             *Key,
  RedfishBios_V1_2_0_ResetBios_CS  **Dst
  )
{
  RedfishCS_status  Status;
  json_t            *TempJsonObj;
...
GenOemActionsCs (
  ...
  json_t                            *JsonObj,
  char                              *Key,
  RedfishBios_V1_2_0_OemActions_CS  **Dst
  )
{
  RedfishCS_status                  Status;
...
GenActionsCs (
  ...
  json_t                         *JsonObj,
  char                           *Key,
  RedfishBios_V1_2_0_Actions_CS  **Dst
  )
{
  RedfishCS_status  Status;
  json_t            *TempJsonObj;
...
GenAttributesCs (
  ...
  json_t                            *JsonObj,
  char                              *Key,
  RedfishBios_V1_2_0_Attributes_CS  **Dst
  )
{
  RedfishCS_status                  Status;
  RedfishCS_Type_JSON_Data          *CsTypeJson;
...
GenLinksCs (
  ...
  json_t                       *JsonObj,
  char                         *Key,
  RedfishBios_V1_2_0_Links_CS  **Dst
  )
{
  RedfishCS_status  Status;
  json_t            *TempJsonObj;
...
static RedfishCS_status
CS_To_JSON_ActionsBios_ChangePassword (
  json_t                                *CsJson,
  char                                  *Key,
  RedfishBios_V1_2_0_ChangePassword_CS  *CSPtr
  )
{
  json_t  *CsParentJson;
  ...
  // target
  if (InsertJsonStringObj (CsJson, "target", CSPtr->target) != RedfishCS_status_success) {
    goto Error;
  }

  // title
  if (InsertJsonStringObj (CsJson, "title", CSPtr->title) != RedfishCS_status_success) {
    goto Error;
  }

  // Set to parent JSON object.
  if (json_object_set_new (CsParentJson, Key, CsJson) == -1) {
    goto Error;
  }
...
static RedfishCS_status
CS_To_JSON_ActionsBios_ResetBios (
  json_t                           *CsJson,
  char                             *Key,
  RedfishBios_V1_2_0_ResetBios_CS  *CSPtr
  )
{
  json_t  *CsParentJson;
...
static RedfishCS_status
CS_To_JSON_ActionsOem (
  json_t                            *CsJson,
  char                              *Key,
  RedfishBios_V1_2_0_OemActions_CS  *CSPtr
  )
{
  json_t  *CsParentJson;
...
CS_To_JSON_Actions (
  json_t                         *CsJson,
  char                           *Key,
  RedfishBios_V1_2_0_Actions_CS  *CSPtr
  )
{
  json_t  *CsParentJson;
  ...
  // #Bios.ChangePassword
  if (CS_To_JSON_ActionsBios_ChangePassword (CsJson, "#Bios.ChangePassword", CSPtr->Bios_ChangePassword) != RedfishCS_status_success) {
    goto Error;
  }

  // #Bios.ResetBios
  if (CS_To_JSON_ActionsBios_ResetBios (CsJson, "#Bios.ResetBios", CSPtr->Bios_ResetBios) != RedfishCS_status_success) {
    goto Error;
  }

  // Oem
  if (CS_To_JSON_ActionsOem (CsJson, "Oem", CSPtr->Oem) != RedfishCS_status_success) {
    goto Error;
  }
...
static RedfishCS_status
CS_To_JSON_Attributes (
  json_t                            *CsJson,
  char                              *Key,
  RedfishBios_V1_2_0_Attributes_CS  *CSPtr
  )
{
  // json_t *CsParentJson;
...
CS_To_JSON_Links (
  json_t                       *CsJson,
  char                         *Key,
  RedfishBios_V1_2_0_Links_CS  *CSPtr
  )
{
  json_t  *CsParentJson;
  ...
  // ActiveSoftwareImage
  if (InsertJsonLinkObj (CsJson, "ActiveSoftwareImage", &CSPtr->ActiveSoftwareImage) != RedfishCS_status_success) {
    goto Error;
  }

  // Oem
  if (CS_To_JSON_LinksOem (CsJson, "Oem", CSPtr->Oem) != RedfishCS_status_success) {
    goto Error;
  }

  // SoftwareImages
  if (InsertJsonLinkArrayObj (CsJson, "SoftwareImages", &CSPtr->SoftwareImages) != RedfishCS_status_success) {
    goto Error;
  }

  // SoftwareImages@odata.count
  if (InsertJsonInt64Obj (CsJson, "SoftwareImages@odata.count", CSPtr->SoftwareImagesodata_count) != RedfishCS_status_success) {
    goto Error;
  }
...
//
RedfishCS_status
CS_To_Bios_V1_2_0_JSON (
  RedfishBios_V1_2_0_Bios_CS  *CSPtr,
  ...
  )
{
  ...
  // Actions
  if (CS_To_JSON_Actions (CsJson, "Actions", CSPtr->Actions) != RedfishCS_status_success) {
    goto Error;
  }

  // AttributeRegistry
  if (InsertJsonStringObj (CsJson, "AttributeRegistry", CSPtr->AttributeRegistry) != RedfishCS_status_success) {
    goto Error;
  }

  // Attributes
  if (CS_To_JSON_Attributes (CsJson, "Attributes", CSPtr->Attributes) != RedfishCS_status_success) {
    goto Error;
  }

  // Description
  if (InsertJsonStringObj (CsJson, "Description", CSPtr->Description) != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // Links
  if (CS_To_JSON_Links (CsJson, "Links", CSPtr->Links) != RedfishCS_status_success) {
    goto Error;
  }

  // Name
  if (InsertJsonStringObj (CsJson, "Name", CSPtr->Name) != RedfishCS_status_success) {
    goto Error;
  }

  // Oem
  if (CS_To_JSON_Oem (CsJson, "Oem", CSPtr->Oem) != RedfishCS_status_success) {
    goto Error;
  }

  // ResetBiosToDefaultsPending
  if (InsertJsonBoolObj (CsJson, "ResetBiosToDefaultsPending", CSPtr->ResetBiosToDefaultsPending) != RedfishCS_status_success) {
    goto Error;
  }

  // @odata.context
  if (InsertJsonStringObj (CsJson, "@odata.context", CSPtr->odata_context) != RedfishCS_status_success) {
    goto Error;
  }

  // @odata.etag
  if (InsertJsonStringObj (CsJson, "@odata.etag", CSPtr->odata_etag) != RedfishCS_status_success) {
    goto Error;
  }

  // @odata.id
  if (InsertJsonStringObj (CsJson, "@odata.id", CSPtr->odata_id) != RedfishCS_status_success) {
    goto Error;
  }

  // @odata.type
  if (InsertJsonStringObj (CsJson, "@odata.type", CSPtr->odata_type) != RedfishCS_status_success) {
    goto Error;
  }
...
//
RedfishCS_status
Json_Bios_V1_2_0_To_CS (
  RedfishCS_char              *JsonRawText,
  RedfishBios_V1_2_0_Bios_CS  **ReturnedCs
  )
{
  RedfishCS_status            Status;
  json_t                      *JsonObj;
